/* Connects to WiFi and tests publishing and subscribing to an MQTT broker.
  ...
  How to use:  
    ...
    Specify MQTT broker.
    Use an MQTT client on a PC to publish to the SUBSCRIBE_TOPIC below, and the message will display on the Serial Monitor.
    Use an MQTT client on a PC to subscribe to the PUBLISH_TOPIC below, and view the message sent by this program.
...
void loop() {
  ...
  mqtt_simple.publish(PUBLISH_TOPIC, msg_pub);  // Publish to a topic
